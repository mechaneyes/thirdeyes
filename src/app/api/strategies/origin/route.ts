...
import { originPrimary } from "../prompts";
    ...
    start(controller) {
      async function processMessages() {
        try {
          ...
          const enhancedPrompt = `${originPrimary.content}\n\nContext:\n"""\n${wikipediaContext}\n"""`;
          ...
          const primaryCompletion = await openai.beta.chat.completions.parse({
            ...
            messages: [
              { role: "system", content: originPrimary.content },
              ...validMessages.map((msg) => ({
                role: msg.role || "user",
                content: msg.content,
              })),
            ],
            response_format: zodResponseFormat(ArtistOrigins, "result"),
            temperature: 0.9,
            max_tokens: 4095,
            top_p: 1,
            frequency_penalty: 0,
